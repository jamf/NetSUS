...
if [[ $detectedOS == 'CentOS' ]] || [[ $detectedOS == 'RedHat' ]]; then
	if ! rpm -qa "epel-release" | grep -q "epel-release" ; then
		yum --disablerepo=\* localinstall -y -q "http://dl.fedoraproject.org/pub/epel/6/x86_64/epel-release-6-8.noarch.rpm" >> $logFile
	fi
	if ! rpm -qa "netatalk" | grep -q "netatalk" ; then
		yum install netatalk -y -q >> $logFile
	fi
