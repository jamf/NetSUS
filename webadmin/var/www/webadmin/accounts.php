...
foreach(explode("\n", $users_str) as $entry) {
	...
	if ($sys_user['name'] != $conf->getSetting("shelluser") && ($sys_user['uid'] < $uid_min || $sys_user['uid'] > $uid_max)) {
...
?>
			...
			<script type="text/javascript">
				...
				function validLdap() {
					...
					if (/^(?=.{2,63}$)(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z0-9]|[A-Za-z0-9][A-Za-z0-9\-]*[A-Za-z0-9])$/.test(ldapdomain.value)) {
						hideError(ldapdomain, 'ldapdomain_label');
					} else {
						showError(ldapdomain, 'ldapdomain_label');
					}
					if (/^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$|^(?=.{1,253}$)(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z0-9]|[A-Za-z0-9][A-Za-z0-9\-]*[A-Za-z0-9])$/.test(ldaphost.value)) {
				...
				function validSysUser() {
					...
					if (sysusernewuid.value == parseInt(sysusernewuid.value) && (sysuids.indexOf(parseInt(sysusernewuid.value)) == -1 || sysusernewuid.value == sysuseruid.value) && (sysusernewlogin.value == '<?php echo $conf->getSetting("shelluser"); ?>' || sysusernewuid.value >= sysuidmin && sysusernewuid.value <= sysuidmax)) {
					...
					if (sysusernewuid.value == parseInt(sysusernewuid.value) && (sysuids.indexOf(parseInt(sysusernewuid.value)) == -1 || sysusernewuid.value == sysuseruid.value) && (sysusernewlogin.value == '<?php echo $conf->getSetting("shelluser"); ?>' || sysusernewuid.value >= sysuidmin && sysusernewuid.value <= sysuidmax) && /^[a-z_][a-z0-9_-]{1,31}$/.test(sysusernewlogin.value) && (syslogins.indexOf(sysusernewlogin.value) == -1 || sysusernewlogin.value == sysuserlogin.value) && /^[^:]{0,128}$/.test(sysusergecos.value) && (sysusertype.value == "Sharing" || sysusertype.value != "Sharing" && /^(\/)[^\0:]*$/.test(sysuserhome.value) && sysuserhome.value != "/dev/null")) {
